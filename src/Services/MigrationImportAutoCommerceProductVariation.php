...
class MigrationImportAutoCommerceProductVariation extends MigrationImportAutoBase {
  ...
  public function buildDataRows(array $row, array &$data_rows) {
    ...
    // Set type
    $data_rows[$k]['type'] = $row['relationships']['commerce_product_variation_type']['data']['meta']["drupal_internal__target_id"];
  ...
  public function buildMappingProcess($configuration, array &$process) {
    if (!empty($configuration['source']['data_rows'][0])) {
      foreach ($configuration['source']['data_rows'][0] as $fieldName => $value) {
        if ($fieldName == 'drupal_internal__variation_id') {
          $process['variation_id'] = $fieldName;
        }
